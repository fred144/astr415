...
#include "./given_scripts/lf2.c"
#include "./given_scripts/nrutil.c"
#include "./given_scripts/nrutil.h"

void lf2(
    float y[], float dydx[], int n, float x, float h, float yout[], 
...
void flattened_grav_deriv(float t, float vals[], float val_derivs[] ){
    ...
    for (int primary_p = 0;  primary_p < N_particles ;  primary_p++){
        ...
        // vals has the form x vx y vy z vz
        // deriv has the form vx ax vy ay vz az

        val_derivs[primary_p*params + 1] = vals[primary_p*params + 2];
        val_derivs[primary_p*params + 3] = vals[primary_p*params + 4];
        val_derivs[primary_p*params + 5] = vals[primary_p*params + 6];
    
        val_derivs[primary_p*params + 2]  = 0.0;
        ...
        for (int secondary_p = 0; secondary_p < N_particles; secondary_p++){ 
            ...
            if (primary_p != secondary_p){
                ...
                y_diff = vals[primary_p*params + 3] - vals[secondary_p*params + 3];
                z_diff = vals[primary_p*params + 5] - vals[secondary_p*params + 5];
                mag_diff_cubed = powf(x_diff*x_diff  + y_diff*y_diff + z_diff*z_diff, 1.5);
                val_derivs[primary_p*params + 2] += -(G * s_mass * x_diff) / mag_diff_cubed;
                val_derivs[primary_p*params + 4] += -(G * s_mass * y_diff) / mag_diff_cubed;
...
int main(int arg_count, char **argv){
    ...
    // naming scheme 
    char filename[ 200];
    sprintf(filename, "./data_frog/frog_out_%04d.txt", 0);
    ...
    printf("%3.10e %3.10e %3.10e %3.10e %3.10e %3.10e %3.10e\n", time, vals[1], vals[3], vals[5], vals[2], vals[4], vals[6]);
    printf("%3.10e %3.10e %3.10e %3.10e %3.10e %3.10e %3.10e\n", time, vals[7], vals[9], vals[11], vals[8], vals[10], vals[12]);
	...
	for(int n=1; n<=Nsteps; n++){
        ...
        lf2(vals, val_derivs, Npar*Nparams, time, step, vals, *flattened_grav_deriv);

        time = n * step;
       
        // make the filename for this snapshot 
        sprintf(filename, "./data_frog/frog_out_%04d.txt", n);
